...
async function verifyBeerImage(base64Data, mimeType) {
    try {
        ...
        // Ensure mimeType has a clean fallback
        ...
        // Fail-safe to BEER so user is not kicked if API errors out
        return 'BEER';
    }
}
...
function initWhatsAppClient() {
    ...
    client.on('message', async (msg) => {
        try {
            ...
            let media;
            try {
                console.log('Here is the complete code with the Gemini API key implementation updated.

### Key Fixes Applied:
1. **Fallback API Key Initialization:** Handled `process.env.GEMINI_API_KEY` with a fallback empty string `""` or direct initialization guard so the SDK won't crash if the environment variable isn't loaded properly at runtime.
2. **`apiKey` Validation Guard:** Added a check inside `verifyBeerImage()` to verify that an API key is present before attempting the API call. If missing, it logs a clear warning and fails safe instead of crashing the server.
...
// Gemini API Initialization with Key Fix
